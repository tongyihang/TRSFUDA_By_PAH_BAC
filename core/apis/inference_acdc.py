    ...
    for b in range(bs):
        ...
        if 'cityscapes' in cfg.DATASETS.TARGET_TRAIN:
            ...
            img_path = os.path.join(image_root,
                                    "bdd-100k/bdd100k/images/10k/train/%s" % selected_name.split('.')[0] + '.jpg')
...
def run_candidate(cfg, feature_extractor, classifier, local_rank, distributed, init_candidate=False, update_meta=False):
    ...
    if init_candidate:
        """
        gta-->citysacpes         city:1024X2048; acdc:1080X1920
        predicted_label = np.zeros((len(test_loader), 256, 512))
        predicted_prob = np.zeros((len(test_loader), 256, 512))
        """
        #cityscapes->acdc
        predicted_label = np.zeros((len(test_loader), 270, 480))
        predicted_prob = np.zeros((len(test_loader), 270, 480))
    else:
        predicted_label = np.zeros((len(test_loader), 540, 960))
    ...
    with torch.no_grad():
        for i, (x, y, name,) in tqdm(enumerate(test_loader)):
            ...
            pred = output.max(1)[1]
            intersection, union, target = intersectionAndUnionGPU(pred.clone(), y, cfg.MODEL.NUM_CLASSES,
                                                                  cfg.INPUT.IGNORE_LABEL)
            if distributed:
                torch.distributed.all_reduce(intersection), torch.distributed.all_reduce(
                    union), torch.distributed.all_reduce(target)
            ...
            if init_candidate:
                prob = probs.max(1)[0]
                predicted_prob[i] = F.interpolate(prob.unsqueeze(0).float(), size=[270, 480]).cpu().numpy().squeeze()
                predicted_label[i] = F.interpolate(pred.unsqueeze(0).float(), size=[270, 480]).cpu().numpy().squeeze()
            else:
                single_iou = intersection / (union + 1e-8)
                single_iou_list[i] = single_iou
                predicted_label[i] = F.interpolate(pred.unsqueeze(0).float(),
    ...
    if init_candidate:
        ...
        for index in range(len(name_list)):
            ...
            mask_filename = name[0].replace('/','_')
        ...
        for i in range(cfg.MODEL.NUM_CLASSES):
            ...
            if len(x) == 0:
                ...
                thres.append(x[int(np.round(len(x) * cfg.TCR.TOPK_CANDIDATE))])
                inx = min(int(np.round(len(x) * 0.9)),len(x)-1)
                meta_thres.append(x[inx])
        thres = np.array(thres)
        meta_thres = np.array(meta_thres)
    ...
    if local_rank == 0:
        ...
        for i in range(cfg.MODEL.NUM_CLASSES):
            logger.info(
                'Class_{} {} Result: iou/accuracy {:.4f}/{:.4f}.'.format(i, test_data.trainid2name[i], iou_class[i],
                                                                         accuracy_class[i]))
            print('Class_{} {} Result: iou/accuracy {:.4f}/{:.4f}.'.format(i, test_data.trainid2name[i], iou_class[i],
                                                                           accuracy_class[i]))
