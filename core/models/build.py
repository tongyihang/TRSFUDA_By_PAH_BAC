...
from .classifier import ASPP_Classifier_V2,ASPP_Classifier_WORSE
...
"""def build_classifier(cfg):
    ...
    return classifier
"""
def build_classifier(cfg,default=0):
    if default == 0:
        _, backbone_name = cfg.MODEL.NAME.split('_')
    elif default == 1:
        backbone_name = cfg.MODEL.WORSE
    if backbone_name.startswith('vgg'):
        classifier = ASPP_Classifier_V2(1024, [6, 12, 18, 24], [6, 12, 18, 24], cfg.MODEL.NUM_CLASSES)
    elif backbone_name.startswith('resnet'):
        classifier = ASPP_Classifier_V2(2048, [6, 12, 18, 24], [6, 12, 18, 24], cfg.MODEL.NUM_CLASSES)
    elif backbone_name.startswith('worse'):
        classifier = ASPP_Classifier_WORSE(2048, [6, 12, 18, 24], [6, 12, 18, 24], cfg.MODEL.NUM_CLASSES)
    else:
        raise NotImplementedError
    return classifier
