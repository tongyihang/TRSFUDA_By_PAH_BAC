...
class rand_mixer_v2():
    def __init__(self, cfg, root='/mnt/data/asus/tyh/sfda/cma-main/DATA_DIR/ACDC'):
        self.image_root = root
        self.cdd_root = os.path.join(cfg.OUTPUT_DIR)#/mnt/data/asus/tyh/sfda/SND_FUBEN/results/our_1
        self.class_num = cfg.MODEL.NUM_CLASSES#19
        candidate_name = 'CTR'
        self.candidate_name = candidate_name
        self.file_list = os.path.join(cfg.OUTPUT_DIR, candidate_name + '.p')#results/our_1/CTR.p
        self.label_to_file, _ = pickle.load(open(self.file_list, "rb"))#CTR.p
    ...
    # return : 一次getitem  获取数据 只补偿一个类别的 达到50的
    #       out_imgs:    (w, h, 3)
    #       out_labels:  (w, h, 3)
        ...
        class_idx = np.random.choice(classes, size=1, p=choice_p)[0]#[4]   加[0] 以后变成 4
        ins_pix_num = 50
        while True:#从50%阈值 中挑选label==class_idx 且该类别像素点大于>50个
            name = random.sample(self.label_to_file[class_idx], 1)
            cdd_img_path = os.path.join(self.image_root, "rgb_anon/%s" % name[0])
            cdd_label_path = os.path.join(self.cdd_root, "{}/{}".format(self.candidate_name, name[0].replace('/','_')))
        ...
        seg, forenum = sklabel(masknp, background=0, return_num=True, connectivity=2)#seg 不连通的区域 打上 1 2 3  forenum为不连通区域的个数。
        ...
        for i in range(forenum):
            ...
            #### rescale instance
            randscale = 1.0 + np.random.rand() * 1.5#[1.0, 2.5)
...
class ACDC_B(data.Dataset):
    def __init__(
        self,
        data_root = '/mnt/data/asus/tyh/sfda/cma-main/DATA_DIR/ACDC',
        data_list = 'ACDC_train.txt',#datasets/cityscapes_train_list.txt
        max_iters=None,#62500*4
        num_classes=19, #19
        split="train",#train
        transform=None,#...
        rsc=None,#CTR_0.p
        ...
        self.mixup = cfg.TCR.OPEN#True
        rsc_freq_file = rsc#CTR_O  为了采样
        print('rsc_freq_file', rsc_freq_file) #CTR_0.p 训练过程中没有变过(?)
        ...
        if max_iters is not None:
             ...
             for i in range(self.NUM_CLASS):
                 ind[i] = 0
             for e in range(int(max_iters / SUB_EPOCH_SIZE) + 1):#501
                 ...
                 for i in range(SUB_EPOCH_SIZE):
                     ...
                     w = w / w.sum()
                     c = np.random.choice(self.NUM_CLASS, p=w)#以w向量概率分布取0-18某个类别
                     ...
                     ind[c] = ind[c] + 1
                     for i in self.file_to_label[c_file]:
                         cur_class_dist[i] += self.file_to_label[c_file][i]
             print("------------------------city balance sample-----------------------------")
             self.img_ids = tmp_list
        if self.mixup:
            self.mixer = rand_mixer_v2(cfg)
            self.mix_classes = [4, 5, 6, 7, 12, 16, 17, 18]#our:全部类别补偿
        ...
        for name in self.img_ids:
            if pseudo:
                ...
                self.data_list.append(
                    {
                        "img": os.path.join(
                            self.data_root, "rgb_anon/%s" % (name)
                        ),
                        "label": os.path.join(
                            self.data_root, "gt/%s" % (name.split('_rgb_anon.png')[0] + "_gt_labelTrainIds.png"),
                        ),
                        "name": name,
                    }
                )
    ...
    def __getitem__(self, index):
        ...
        # mixup
        if self.mixup:
            trans_image = denormalizeimage(trans_image.unsqueeze(0))#反归一化
        ...
        return trans_image, torch.from_numpy(np.array(trans_label)), name, trans_param, img_full, mix_label
