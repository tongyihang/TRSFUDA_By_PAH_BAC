...
class DatasetCatalog(object):
    ...
    def get(name, mode, num_classes, max_iters=None, transform=None, rsc=None, cfg=None):
        ...
        if "acdc" in name:#  test  ->true
            return ACDC(max_iters=max_iters, num_classes=num_classes, split=mode, transform=transform)
        if "II" in name:# run_    name II_train    ; mode train    firse_run_candidate->true
            return ACDC(data_list = 'ACDC_train.txt',max_iters=max_iters, num_classes=num_classes, split=mode, transform=transform)
        if "mix_b" in name:#acdc train
            return ACDC_B(data_list='ACDC_train.txt', max_iters=max_iters, num_classes=num_classes, split=mode, transform=transform,rsc=rsc, cfg=cfg)
        if "TT" in name:
            return ACDC(data_list = 'ACDC_train.txt',max_iters=max_iters, num_classes=num_classes, split=mode, transform=transform, cfg=cfg, pseudo=True)
        """
        ...
        if "gta5" in name:
            ...
            data_dir = DatasetCatalog.DATASET_DIR
            attrs = DatasetCatalog.DATASETS[name]
            data_dir01 = "/home/asus/tyh/sfda/SegFormer/data"
            args = dict(
                root=os.path.join(data_dir01, attrs["data_dir"]),
                data_list=os.path.join(data_dir, attrs["data_list"]),
            )
            if 'distill' in name:
