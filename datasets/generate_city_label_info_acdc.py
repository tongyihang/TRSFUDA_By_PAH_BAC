...
def generate_label_info():
    ...
    for labfile in tqdm(labfiles):
        ...
        for lab in label:
            if 255 == lab: continue
            label_to_file[lab].append(
                os.path.join(labfile.split('_')[0], labfile.replace('gtFine_labelIds', 'leftImg8bit')))
            file_to_label[
                os.path.join(labfile.split('_')[0], labfile.replace('gtFine_labelIds', 'leftImg8bit'))].append(lab)
...
def gen_lb_info(cfg, dir_name='CTR', nprocs=1):
    ...
    #data_list = "datasets/cityscapes_train_list.txt"  原来的
    data_list = "ACDC_train.txt"  #acdc
    with open(data_list, "r") as handle:
        imglist = handle.readlines()
    labdir = os.path.join(savedir, dir_name)#result/CTR
    ...
    if dir_name == 'CTR':
        ...
        def generate_label_info():
            ...
            for labfile in tqdm(labfiles):
                ...
                for lab in label:
                    ...
                    name = labfile.split('_')
                    c = name[0] + "/" + name[1] + '/' + name[2] + '/' + name[3] + '_' + name[4] + '_' + name[5] + '_' + \
                        name[6] + '_' + name[7]
                    label_to_file[lab].append(c)  #aachen_000000_000019_leftImg8bit.png -->GOPR0122_frame_000161_rgb_anon.png
                        #aachen/aachen_000000_000019_leftImg8bit.png
                    file_to_label[c].append(lab)
        ...
        label_to_file = [[] for _ in range(len(id_to_trainid.keys()))]
        file_to_label = {e.strip(): {} for e in imglist}  # 改成 dict 存每类像素数

        def generate_label_info():
            label_to_file = [[] for _ in range(len(id_to_trainid.keys()))]
            file_to_label = {e.strip(): {} for e in imglist}  # 改成 dict 存每类像素数

            for labfile in tqdm(labfiles):#fog_train_GOPR0475_GOPR0475_frame_000041_rgb_anon.png
                label_path = os.path.join(labdir, labfile)
                label_ = np.array(Image.open(label_path), dtype=np.uint8)
                ...
                for lab in unique_labels:
                    if lab == 255:
                        continue
                    pixel_count = np.sum(label_ == lab)
                    if pixel_count < 50:
                        continue

                    trainid = lab  # 如果 lab 就是 trainid（如已经映射过）
                    if trainid >= len(label_to_file):
                        continue

                    label_to_file[trainid].append(c)

                    # 存每类像素数
                    file_to_label[c][trainid] = pixel_count
            return label_to_file, file_to_label
        if nprocs == 1:
            label_to_file, file_to_label = generate_label_info()
        if clsses_num == 19:
